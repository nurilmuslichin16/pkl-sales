...
class Work_order_provi extends MY_Controller
{
    ...
    public function progress()
    {
        ...
        $time         = $get['time'];
        $unit          = $get['unit'];
        $order        = empty($get['order']) ? 'psb' : $get['order'];
        $modtype      = empty($get['modtype']) ? 'all' : str_replace("_", " ", $get['modtype']);

        $data['title']             = 'Progress Order';
        $data['subtitle']         = 'Order ' . time_progress_provi($time) . ' Progress ' . kategori_progress($kategori) . ' Datel ' . $datel . ' Unit ' . strtoupper($unit);
        $data['datas']             = $this->wo_model->provi_progress_view($kategori, $datel, $time, $order, $modtype, $unit)->result_array();
        $data['teknisi']          = $this->wo_model->teknisi($datel)->result_array();
        if ($order == 'addon') {
            $this->load->view('template', [
                'content' => $this->load->view('sales/work_order/addon/all', $data, true)
            ]);
        } else {
            $this->load->view('template', [
...
/* Location: ./application/controllers/sales/Work_order_provi.php */